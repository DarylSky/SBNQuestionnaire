<HTML>

<HEAD>

<script type="text/javascript" src="http://code.jquery.com/jquery-latest.js"></script>
<script src="../Js/Startup.js"></script>
<link rel="stylesheet" type="text/css" href="../CSS/Startup.css" />

<TITLE>SBN Questionnaire</TITLE>

</HEAD>

<BODY>

	<table align="center">

		<tr>
			<td colspan="2"><img src="../Images/Startup/FirstPageBanner.png"
				width="387" height="61"><br />
			<br /></td>

		</tr>
		<tr>

			<td width="10">Title:</td>

			<td width="10"><input type="text" value="" name="username" id="username"
				title="Enter Title of Questionnaire" />
			<td>
		</tr>

		<tr>

			<td width="10">Category:</td>

			<td width="10"><select
				STYLE="width: 228px; height: 100px; font-size: 18;">
					<option>Community</option>
					<option>Customer Feedback</option>
					<option>Demographics</option>
					<option>Education</option>
					<option>Events</option>
					<option>Healthcare</option>
					<option>Human Resources</option>
					<option>Industry Specific</option>
					<option>Just for Fun</option>
					<option>Market Research</option>
					<option>Non-Profit</option>
					<option>Political</option>
					<option>Others</option>
			</select>
			<td>
		</tr>
		<tr>
			<td colspan="2" style="text-align: right"><a href=""
				onclick="javascript:cancelAction();"><input type="image"
					src="../Images/Startup/FirstPageNextBtn.png" width="150"
					height="55"></a></td>

		</tr>

	</table>



</BODY>

</HTML>
